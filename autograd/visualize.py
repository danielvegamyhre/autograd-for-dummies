# Credit: https://github.com/kennysong/minigrad/blob/master/minigrad/visualize.py
    ...
    def _draw_node(node):
        ...
        if node.prev_op is None:
            ...
            node_text = f'''<<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" CELLPADDING="5">
                <TR><TD>value = {node.value:.4f}</TD></TR>
                <TR><TD>grad = {node.grad:.4f}</TD></TR>
                <TR><TD BGCOLOR="#c2ebff"><FONT COLOR="#004261" FACE="Courier" POINT-SIZE="12">{node.prev_op}</FONT></TD></TR>
            </TABLE>>'''
